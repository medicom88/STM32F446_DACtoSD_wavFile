	...
	if(f_open(&myFiles, pathFile, FA_READ) == FR_OK){
		...
		if(f_read(&myFiles, hWavehdr, sizeof(WAVE_HDR__TypeDef), &myReadBytes) == FR_OK){
			...
			if(WaveData.WavRepeatDataFlag == ENABLE_FLAG_BIT){
				if(f_read(&myFiles, WaveData.WavData_8Bit[FILL_BUFF_0], WaveData.WavDataSize, &myReadBytes) == FR_OK){
	...
	if(WaveData.WavHdrClearFlag == ENABLE_FLAG_BIT){
		...
		if(WaveData.WavRepeatDataFlag == ENABLE_FLAG_BIT){
			...
			if(WaveData.WavCrossRepeatDataFlag == FILL_BUFF_0){
				if(f_read(&myFiles, WaveData.WavData_8Bit[FILL_BUFF_1], WaveData.WavDataSize, &myReadBytes) == FR_OK){
